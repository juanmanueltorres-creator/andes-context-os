...
- [ ] **Step 3: Implement strict asset/actor contracts**
...
- Produces `ActorRole`, `MovementActorRef`, `MovementType`, `MovementReviewState`, `Movement`.

- [ ] **Step 1: Write failing movement tests**

Create the movement portion of `tests/test_movements.py`:

```python
import pytest

from andes_context_os.movements import ActorRole, Movement, MovementReviewState, MovementType
...
def test_stage_change_requires_distinct_previous_and_new_state():
    with pytest.raises(ValueError, match="stage_change requires distinct previous_state and new_state"):
        Movement.from_dict(movement_payload(movement_type="stage_change", previous_state="construction", new_state="construction"))


def test_review_state_controls_reviewed_at():
...
Expected: import failure because `andes_context_os.movements` does not exist.

- [ ] **Step 3: Implement movement contracts**

Create `src/andes_context_os/movements.py` with the exact enums:
...
Implement `MovementActorRef` and `Movement` as frozen/slotted dataclasses with the exact spec fields. Use existing helpers and enforce this logic exactly:
...
reviewed_states = {MovementReviewState.REVIEWED, MovementReviewState.REJECTED, MovementReviewState.SUPERSEDED}
...
`observed_at` and non-null `reviewed_at` must pass `require_aware_iso8601()`. `to_dict()` emits every field, including explicit `None` for optional state/review fields.
...
**Interfaces:**
- Consumes common strict-validation helpers.
- Produces `OpportunityStatus` and `OpportunityHypothesis`.
- Same-asset and evidence-difference checks remain cross-object concerns for Task 4.
...
def test_supported_hypothesis_requires_support_and_review_timestamp():
...
Expected: import failure because `andes_context_os.opportunities` does not exist.

- [ ] **Step 3: Implement opportunity contract**
...
Implement `OpportunityHypothesis` as a frozen/slotted dataclass with the exact spec fields. Enforce locally:
...
for field, values in (("trigger_movement_refs", triggers), ("actor_refs", actors), ("supporting_evidence_refs", support)):
    if len(values) != len(set(values)):
        raise ValueError(f"{field} contains duplicates")

reviewed_states = {OpportunityStatus.SUPPORTED, OpportunityStatus.CONTRADICTED, OpportunityStatus.DISCARDED}
...
`created_at` and non-null `reviewed_at` must be timezone-aware. Keep `assumptions` and `missing_context` unchanged on round-trip.
...
**Interfaces:**
- Consumes `Asset`, `Actor`, `Movement`, `EvidenceCandidate`, `OpportunityHypothesis`.
- Produces:
  - `validate_asset_movement_benchmark(*, assets, actors, movements, evidence_candidates, opportunity_hypotheses) -> None`.
  - `load_asset_movement_fixture(base_dir: str | Path)` returning four tuples.
  - `canonical_asset_movement_projection(...) -> dict[str, list[dict[str, Any]]]`.

- [ ] **Step 1: Write failing validator/integration tests with source-backed evidence objects**
...
def evidence(candidate_id, source_id, title, summary, source_reference, scope_id, authority):
    return EvidenceCandidate.from_dict({
        ...
        "quality": {
            ...
            "limitations": ["Public source; interpretation remains bounded to the stated claim."],
            "missing_context": [],
        },
        "payload_ref": None,
        "corroboration_refs": [],
...
These authority values are exact existing `SourceAuthority` enum values; do not invent a `first_party` value.

Append cross-reference tests:
...
def test_validator_rejects_missing_actor_reference():
    ...
    with pytest.raises(ValueError, match="unknown actor_id: actor:missing"):
        validate_asset_movement_benchmark(
            assets=assets, actors=actors, movements=(bad, *movements[1:]),
            evidence_candidates=PUBLIC_EVIDENCE, opportunity_hypotheses=hypotheses,
        )


def test_validator_rejects_same_asset_violation():
    assets, actors, movements, hypotheses = load_valid_objects()
    bad = OpportunityHypothesis.from_dict({**hypotheses[0].to_dict(), "asset_id": "asset:ar:li:cauchari-olaroz"})
    with pytest.raises(ValueError, match="trigger movement belongs to different asset"):
        validate_asset_movement_benchmark(
            assets=assets, actors=actors, movements=movements,
            evidence_candidates=PUBLIC_EVIDENCE, opportunity_hypotheses=(bad, *hypotheses[1:]),
        )
...
def test_supported_hypothesis_requires_evidence_beyond_trigger_evidence():
    ...
    with pytest.raises(ValueError, match="supported hypothesis requires evidence beyond trigger movement evidence"):
        validate_asset_movement_benchmark(
            assets=assets, actors=actors, movements=movements,
            evidence_candidates=PUBLIC_EVIDENCE, opportunity_hypotheses=(supported, *hypotheses[1:]),
        )
```
...
Expected: import failure because the benchmark module is absent.

- [ ] **Step 3: Implement loader, canonical projection and validator**
...
Implement `validate_asset_movement_benchmark()` with explicit ID maps and these checks in order:

1. duplicate asset, actor, movement, evidence and hypothesis IDs fail;
2. every movement asset exists;
3. every movement actor exists;
4. every movement evidence ref exists;
5. every movement lineage ref exists and is not self-referential;
6. every hypothesis asset exists;
7. every hypothesis actor exists;
8. every trigger movement exists and belongs to the hypothesis asset;
9. every supporting evidence ref exists;
10. a `SUPPORTED` hypothesis must have at least one supporting evidence ID outside the union of trigger-movement evidence IDs;
11. a `SUPPORTED` hypothesis whose every trigger movement is `REJECTED` fails.

Use deterministic `ValueError` messages containing IDs/field names only, never source bodies.

- [ ] **Step 4: Commit validator before fixtures**
...
Do not expect the integration test to pass until Task 5 creates the dogfood fixture directory.
...
**Files:**
- Create all four JSON files under `data/dogfood/argentina-lithium/`.
- Modify `tests/test_asset_movement_benchmark.py`.

**Interfaces:**
- Uses exact Atlas refs `project_id:258`, `project_id:137`, `project_id:52`.
- Uses only evidence IDs present in `PUBLIC_EVIDENCE`.
- Produces exactly one reviewed movement and one `proposed` hypothesis per initial asset.

- [ ] **Step 1: Create exact asset fixtures**

`assets.json`:

```json
[
  {"contract_version":"0.1","asset_id":"asset:ar:li:rio-grande-noa","name":"Río Grande / NOA Lithium","asset_type":"mining_project","commodity":"lithium","territorial_scope_ref":"project:atlas-geotech:258","baseline_source_id":"atlas-geotech","baseline_record_ref":"project_id:258","notes":["Atlas Geotech baseline identity only; mutable state is researched separately."]},
  {"contract_version":"0.1","asset_id":"asset:ar:li:hombre-muerto-oeste","name":"Hombre Muerto Oeste","asset_type":"mining_project","commodity":"lithium","territorial_scope_ref":"project:atlas-geotech:137","baseline_source_id":"atlas-geotech","baseline_record_ref":"project_id:137","notes":["Atlas Geotech baseline identity only; mutable state is researched separately."]},
  {"contract_version":"0.1","asset_id":"asset:ar:li:cauchari-olaroz","name":"Cauchari-Olaroz","asset_type":"mining_project","commodity":"lithium","territorial_scope_ref":"project:atlas-geotech:52","baseline_source_id":"atlas-geotech","baseline_record_ref":"project_id:52","notes":["Atlas Geotech baseline identity only; mutable state is researched separately."]}
]
```

- [ ] **Step 2: Create actor fixtures**

`actors.json`:

```json
[
  {"contract_version":"0.1","actor_id":"actor:noa-lithium","canonical_name":"NOA Lithium Brines Inc.","actor_kind":"organization","jurisdiction":"Canada / Argentina","external_refs":["https://www.noalithium.com/"],"notes":[]},
  {"contract_version":"0.1","actor_id":"actor:hidrotec","canonical_name":"Hidrotec S.A.","actor_kind":"organization","jurisdiction":"Argentina","external_refs":[],"notes":["Named by NOA as drilling contractor in the 2026 Rio Grande campaign."]},
  {"contract_version":"0.1","actor_id":"actor:galan-lithium","canonical_name":"Galan Lithium Limited","actor_kind":"organization","jurisdiction":"Australia / Argentina","external_refs":["https://galanlithium.com.au/"],"notes":[]},
  {"contract_version":"0.1","actor_id":"actor:lithium-argentina","canonical_name":"Lithium Argentina AG","actor_kind":"organization","jurisdiction":"Switzerland / Argentina","external_refs":["https://www.lithium-argentina.com/"],"notes":[]},
  {"contract_version":"0.1","actor_id":"actor:ganfeng-lithium","canonical_name":"Ganfeng Lithium","actor_kind":"organization","jurisdiction":"China / Argentina","external_refs":[],"notes":["Named by Lithium Argentina as its partner in the Cauchari-Olaroz Stage 2 development work."]}
]
```

- [ ] **Step 3: Create movement fixtures**
...
[
  {
    "contract_version":"0.1","movement_id":"movement:rg:drilling:2026-05-21","asset_id":"asset:ar:li:rio-grande-noa","movement_type":"drilling","observed_at":"2026-09-02T22:00:00-03:00",
    "actor_refs":[{"actor_id":"actor:noa-lithium","role":"operator","notes":[]},{"actor_id":"actor:hidrotec","role":"contractor","notes":[]}],
    "evidence_candidate_refs":["evidence:rg:noa:2026-05-21"],
    ...
    "previous_state":null,"new_state":null,"review_state":"reviewed","reviewed_at":"2026-09-02T22:05:00-03:00","derived_from_movement_ids":[],
    "limitations":["First-party company disclosure; does not establish future procurement beyond the named work."]
  },
  {
    "contract_version":"0.1","movement_id":"movement:hmw:stage-change:2026-q2","asset_id":"asset:ar:li:hombre-muerto-oeste","movement_type":"stage_change","observed_at":"2026-09-02T22:00:00-03:00",
    "actor_refs":[{"actor_id":"actor:galan-lithium","role":"operator","notes":[]}],
    ...
    "previous_state":"construction / commissioning","new_state":"production ramp-up","review_state":"reviewed","reviewed_at":"2026-09-02T22:05:00-03:00","derived_from_movement_ids":[],
  ...
  {
    "contract_version":"0.1","movement_id":"movement:co:expansion:2026-08-11","asset_id":"asset:ar:li:cauchari-olaroz","movement_type":"expansion","observed_at":"2026-09-02T22:00:00-03:00",
    "actor_refs":[{"actor_id":"actor:lithium-argentina","role":"partner","notes":[]},{"actor_id":"actor:ganfeng-lithium","role":"partner","notes":[]}],
...
- [ ] **Step 4: Create proposed hypothesis fixtures**
...
[
  {
    "contract_version":"0.1","hypothesis_id":"opportunity:rg:external-field-services","asset_id":"asset:ar:li:rio-grande-noa","trigger_movement_refs":["movement:rg:drilling:2026-05-21"],"actor_refs":["actor:noa-lithium"],"need_category":"field_services",
    "statement":"Continued work toward PFS may create additional demand for externally procured drilling, hydrogeological or related field services.","supporting_evidence_refs":[],
    "assumptions":["At least part of future field work would be procured from external providers."],
    "missing_context":["Current supplier roster.","Procurement model and contract duration.","Remaining 2026-2027 field-work scope."],"status":"proposed","created_at":"2026-09-02T22:10:00-03:00","reviewed_at":null
  },
  {
    "contract_version":"0.1","hypothesis_id":"opportunity:hmw:ramp-up-support","asset_id":"asset:ar:li:hombre-muerto-oeste","trigger_movement_refs":["movement:hmw:stage-change:2026-q2"],"actor_refs":["actor:galan-lithium"],"need_category":"ramp_up_support",
    "statement":"Production ramp-up and planned capacity growth may create service needs around operating optimisation, laboratory QA/QC, pond works or plant support.","supporting_evidence_refs":[],
    "assumptions":["Some ramp-up or expansion scopes may be externally sourced rather than fully internal."],
    "missing_context":["Current operating contractor model.","Laboratory and QA/QC supplier arrangements.","Scope owners for planned pond works."],"status":"proposed","created_at":"2026-09-02T22:10:00-03:00","reviewed_at":null
  },
  {
    "contract_version":"0.1","hypothesis_id":"opportunity:co:stage2-supplier-scope","asset_id":"asset:ar:li:cauchari-olaroz","trigger_movement_refs":["movement:co:expansion:2026-08-11"],"actor_refs":["actor:lithium-argentina","actor:ganfeng-lithium"],"need_category":"expansion_services",
    "statement":"Stage 2 early development may create externally addressable scopes around wells, infrastructure, site preparation, engineering or equipment integration.","supporting_evidence_refs":[],
    "assumptions":["At least one Stage 2 scope may be open to external suppliers or contractors."],
    "missing_context":["Stage 2 procurement model.","Existing framework agreements.","Packages retained internally by Ganfeng or Minera Exar."],"status":"proposed","created_at":"2026-09-02T22:10:00-03:00","reviewed_at":null
  }
]
```

- [ ] **Step 5: Add exact baseline, evidence and order-independence assertions**
...
def test_canonical_projection_is_order_independent():
    ...
    first = canonical_asset_movement_projection(assets=assets, actors=actors, movements=movements, opportunity_hypotheses=hypotheses)
    second = canonical_asset_movement_projection(
        assets=tuple(reversed(assets)), actors=tuple(reversed(actors)), movements=tuple(reversed(movements)), opportunity_hypotheses=tuple(reversed(hypotheses)),
    )
    assert first == second
```

- [ ] **Step 6: Run integration and focused suite, then commit**
...
Expected: both pytest commands PASS before commit.
...
**Files:**
- Modify `README.md` and `tests/test_asset_movement_benchmark.py`.
- Verify `pyproject.toml` remains unchanged in dependency semantics.

- [ ] **Step 1: Add privacy/dependency tests**
...
Run `pytest -q tests/test_asset_movement_benchmark.py` and fix fixture wording if the privacy gate fails; do not weaken the gate.

- [ ] **Step 2: Add README V0.4 section**
...
Expected:
- full legacy + V0.4 suite PASS;
- `dependency gate: PASS`;
- no whitespace errors;
- fixture diff contains structured summaries/IDs only, no copied source bodies or private/action data;
- no unintended files.

- [ ] **Step 4: Commit release boundary**
...
Do not claim V0.4 complete unless the fresh output shows all tests green and no unintended working-tree changes.

## Implementation Completion Criteria

- `Asset` preserves identity/baseline fields only and rejects mutable `current_stage`-style additions.
- `Actor` has no durable commercial role field.
- every benchmark `Movement` resolves an asset, actor-role refs and at least one `EvidenceCandidate`.
- every benchmark hypothesis resolves its trigger movement(s) and actors.
- all three public dogfood hypotheses remain `proposed`; no service demand is represented as confirmed.
- supported-state tests prove additional evidence beyond trigger evidence is required.
- exact Atlas refs remain `project_id:258`, `project_id:137`, `project_id:52`.
